...
vencimento = st.sidebar.text_input("Data do 1º Vencimento", value=datetime.today().strftime('%d/%m/%Y')) # Default para hoje
...
if st.sidebar.button("Gerar Proposta Profissional"):
    if not razao_social or not consultor:
        ...
        if not os.path.exists(bg_image_path):
            ...
            # NOVO: Imagem de fundo no cabeçalho
            pdf.image(bg_image_path, 0, 0, w=210, h=45) 

            # Estilo Texto Cabeçalho Dr. Fiscal (Mantido)
            ...
            # Reset para texto preto (Crucial: a cor do texto padrão deve ser preta fora do cabeçalho)
